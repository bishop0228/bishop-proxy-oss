/**
 * Behavioral probes for §1.17.18 Azure OpenAI api-key BYOK leg (/byok/azure/...).
 *
 * 9 probes:
 *   (1) managed → 400 azure_requires_byok; mock NOT hit
 *   (2) byok_key_missing (no header + no-colon cred); mock NOT hit
 *   (3) byte-exact body forwarded (no re-serialization)
 *   (4) api-key header===test key; authorization header ABSENT; ?api-version preserved
 *   (5) classify-block → 451; Azure upstream NOT called   [unit-style]
 *   (6) Pillar-1 audit absence: no logEvent contains apiKey or raw cred  [unit-style]
 *   (7) azure_resource_invalid: multi-label resource → 400; mock NOT hit
 *   (8) UNIT isAnchoredEnterpriseHost: valid/invalid patterns; valid not in exact set
 *   (9) shared choke: missing bearer → 401; quota 429 → quota_exceeded + X-Bishop-Cap-Type
 */
...
import { handleAzure } from "../src/routes/azure";
...
const TEST_RESOURCE = "myresource";
const TEST_API_KEY = "azure-test-api-key-1234";
const UPSTREAM_KEY = `${TEST_RESOURCE}:${TEST_API_KEY}`;
...
async function enroll(
  ...
  const enrollBody: Record<string, string> = {
    ...
    counter,
    fingerprint_hash: fp,
    client_version: "test-azure-0.1.0",
  };
...
describe("Azure BYOK leg (/byok/azure/...)", () => {
  ...
  beforeAll(async () => {
    mock = await unstable_dev("test/mock-azure-upstream.ts", {
    ...
    // Avoid a-d.repeat(64): reserved by enroll.test.ts (shared KV state on disk).
    managedToken = await enroll(worker, "7".repeat(64));
    byokToken = await enroll(worker, "8".repeat(64), "byok");
  }, 60000);
  ...
  // ── Probe 1: managed → 400 azure_requires_byok; mock NOT hit ──────────

  it("managed token: /byok/azure/... → 400 azure_requires_byok; mock NOT hit", async () => {
    ...
    expect(body.error).toBe("azure_requires_byok");
  ...
  it("byok + no upstream key OR no-colon cred → 400 byok_key_missing; mock NOT hit", async () => {
    // 2a: absent header
    const res1 = await worker.fetch("/byok/azure/openai/deployments/gpt-4/chat/completions", {
      method: "POST",
      headers: {
        "content-type": "application/json",
    ...
    expect(b1.error).toBe("byok_key_missing");

    // 2b: no-colon credential
    const res2 = await worker.fetch("/byok/azure/openai/deployments/gpt-4/chat/completions", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        authorization: `Bearer ${byokToken}`,
        "x-bishop-upstream-key": "justakeynocolon",
      },
      body: TEST_BODY,
    });
    expect(res2.status).toBe(400);
    const b2 = (await res2.json()) as { error: string };
    expect(b2.error).toBe("byok_key_missing");
  ...
  // ── Probe 4: api-key header + no authorization + ?api-version preserved

  it("byok: api-key===test key; authorization ABSENT; ?api-version preserved", async () => {
    const res = await worker.fetch(
      "/byok/azure/openai/deployments/gpt-4/chat/completions?api-version=2024-02-01",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    const [apiKeyResp, authResp, searchResp] = await Promise.all([
      mock.fetch(mockUrl + "/__last_apikey").then((r) => r.json() as Promise<{ apiKey: string | null }>),
      mock.fetch(mockUrl + "/__last_auth").then((r) => r.json() as Promise<{ auth: string | null }>),
      mock.fetch(mockUrl + "/__last_search").then((r) => r.json() as Promise<{ search: string | null }>),
    ]);

    // api-key header must equal the test key.
    expect(apiKeyResp.apiKey).toBe(TEST_API_KEY);
    // Authorization header must be absent at the upstream.
    expect(authResp.auth).toBeNull();
    // ?api-version must have been forwarded.
  ...
  it("classify block: 451 content_policy_violation; Azure upstream NOT called", async () => {
    ...
    try {
      const req = new Request(
        "http://proxy/byok/azure/openai/deployments/gpt-4/chat/completions",
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
      ...
      const resp = await handleAzure(req, makeBlockEnv(), makeCtx());
      ...
      const azureCalls = fetchSpy.mock.calls.filter(([input]) => {
        const url = String(input instanceof Request ? input.url : input);
        return url.includes("openai.azure.com");
      });
      expect(azureCalls).toHaveLength(0);

      // Classification block log event must have been emitted.
  ...
  it("Pillar-1: no logEvent contains apiKey or raw credential substring", async () => {
    ...
    try {
      ...
      const resp = await handleAzure(req, makeAllowEnv(), makeCtx());
  ...
  // ── Probe 7: azure_resource_invalid (multi-label resource) ───────────

  it("multi-label resource → 400 azure_resource_invalid; mock NOT hit", async () => {
    const multiLabelKey = "foo.evil:myapikey";
    const res = await worker.fetch("/byok/azure/openai/deployments/gpt-4/chat/completions", {
      method: "POST",
      headers: {
        "content-type": "application/json",
    ...
    expect(res.status).toBe(400);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe("azure_resource_invalid");
  ...
  // ── Probe 8: isAnchoredEnterpriseHost unit assertions ─────────────────

  it("isAnchoredEnterpriseHost: valid/invalid patterns; valid Azure host not in exact set", () => {
    // Accepts — single label + exact suffix.
    expect(isAnchoredEnterpriseHost("myresource.openai.azure.com")).toBe(true);
    expect(isAnchoredEnterpriseHost("my-resource123.openai.azure.com")).toBe(true);

    // Rejects — suffix-bypass (extra trailing label).
    expect(isAnchoredEnterpriseHost("foo.openai.azure.com.attacker.com")).toBe(false);
    // Rejects — multi-label resource (dots in label).
    expect(isAnchoredEnterpriseHost("foo.evil.com.openai.azure.com")).toBe(false);
    // Rejects — unrelated domain.
    expect(isAnchoredEnterpriseHost("evil.com")).toBe(false);
    // Rejects — empty label (leading dot).
    expect(isAnchoredEnterpriseHost(".openai.azure.com")).toBe(false);
    // Rejects — leading hyphen.
    expect(isAnchoredEnterpriseHost("-resource.openai.azure.com")).toBe(false);
    // Rejects — uppercase (no `i` flag).
    expect(isAnchoredEnterpriseHost("MYRESOURCE.openai.azure.com")).toBe(false);

    // Valid Azure host is NOT in the exact ALLOWED_OUTBOUND_HOSTS set —
    // it's served by the enterprise-host floor, not the static list.
    const allowedSet = new Set<string>(ALLOWED_OUTBOUND_HOSTS as readonly string[]);
    expect(allowedSet.has("myresource.openai.azure.com")).toBe(false);
  });

  // ── Probe 9: shared choke ─────────────────────────────────────────────

  it("shared choke: missing bearer → 401 missing_bearer; quota 429 → quota_exceeded + X-Bishop-Cap-Type", async () => {
    // 9a — missing bearer (integration).
    const res = await worker.fetch("/byok/azure/openai/deployments/gpt-4/chat/completions", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: TEST_BODY,
    ...
    try {
      ...
      const resp = await handleAzure(req, makeQuota429Env(), makeCtx());
