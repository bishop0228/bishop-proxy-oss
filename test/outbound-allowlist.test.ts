...
describe("outbound-allowlist: §1.17.18 enterprise-host floor guards", () => {
  ...
  it("8c: ALLOWED_OUTBOUND_HOSTS.length === 91 — 42 provider + 44 MCP + 1 model-registry + 3 S6b worker-egress + 1 HF (W38-S968 +1 novita)", () => {
    expect(ALLOWED_OUTBOUND_HOSTS.length).toBe(91);
  });
  ...
  it("9c: ALLOWED_OUTBOUND_HOSTS.length === 91 — 42 provider + 44 MCP + 1 model-registry + 3 S6b worker-egress + 1 HF (exact-match; W38-S968 +1 novita)", () => {
    expect(ALLOWED_OUTBOUND_HOSTS.length).toBe(91);
  });
});
...
describe("outbound-allowlist: W38-S868 §9.3.8c HuggingFace BYO-model guards", () => {
  ...
  it("12d: the HF CDN host is NOT an exact-allowlist entry (length stays 91 = pattern-only)", () => {
...
describe("outbound-allowlist: W38-S735 per-account MCP host pattern guards", () => {
  ...
  it("11c: the per-account hosts are NOT added to ALLOWED_OUTBOUND_HOSTS (length 91 = exact-match only)", () => {
    expect(ALLOWED_OUTBOUND_HOSTS.length).toBe(91);
    for (const host of ENTERPRISE_OK) {
