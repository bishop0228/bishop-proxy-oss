/**
 * POST /v1/gemini/chat/completions — Google Gemini OpenAI-compatible upstream route.
 *
 ...
 * the Gemini leg carries no tee()/SSE-usage parsing and no cost meter — quota
...
export async function handleGemini(
  ...
  const upstreamUrl = env.GEMINI_BASE_URL ?? "https://generativelanguage.googleapis.com/v1beta/openai";
  const upstream = await fetchWithRetry(`${upstreamUrl}/chat/completions`, {
    method: "POST",
