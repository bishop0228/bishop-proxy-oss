/**
 * POST /byok/vertex/<path> — Google Vertex AI Bearer distinct-auth BYOK leg (§1.17.19).
 *
 ...
 *   Step 6: BYOK-only entitlement gate — managed fails closed (vertex_requires_byok, 400).
 *          Credential format: x-bishop-upstream-key = "<region>:<accessToken>".
 *          Region validated via isAnchoredEnterpriseHost; vertex_region_invalid on failure.
 *   Step 7: Authorization: Bearer upstream fetch — host constructed server-side from region;
 *          url.search preserved; api-key header NOT used (Vertex uses Google OAuth Bearer).
 *
 * Pillar 1 invariant: region and accessToken are NEVER recorded in any ProxyLogEvent.
...
export async function handleVertex(
  ...
  const parts = url.pathname.split("/"); // ["", "byok", "vertex", ...]
  ...
  // Step 6 — entitlement gate. Vertex is BYOK-only; managed fails closed.
  const accountMode = (record.account_mode ?? "managed") as "managed" | "byok";
  if (accountMode !== "byok") {
    emitError(requestId, ip, requestSize, 400, "vertex_requires_byok", startedAt, tokenId);
    return jsonError(400, "vertex_requires_byok");
  }
  ...
  // Credential format: "<region>:<accessToken>"
  const colonIdx = upstreamKeyRaw.indexOf(":");
  if (colonIdx < 1) {
    emitError(requestId, ip, requestSize, 400, "byok_key_missing", startedAt, tokenId);
    return jsonError(400, "byok_key_missing");
  }
  const region = upstreamKeyRaw.slice(0, colonIdx).toLowerCase();
  const accessToken = upstreamKeyRaw.slice(colonIdx + 1);
  ...
  // interceptor if the region somehow bypasses this early gate.
  const vertexHost = `${region}${VERTEX_UPSTREAM.hostSuffix}`;
  if (!isAnchoredEnterpriseHost(vertexHost)) {
    emitError(requestId, ip, requestSize, 400, "vertex_region_invalid", startedAt, tokenId);
    return jsonError(400, "vertex_region_invalid");
  }

  // Step 7 — Authorization: Bearer upstream fetch.
  // host constructed server-side; url.search preserved (?alt= etc.).
  // NO api-key header — Vertex uses Google OAuth Bearer auth.
