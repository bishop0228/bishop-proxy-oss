/**
 * POST /v1/qwen/chat/completions — Alibaba Qwen OpenAI-compatible upstream route.
 *
 ...
 * the Qwen leg carries no tee()/SSE-usage parsing and no cost meter — quota
 ...
 *
 * Entitlement gate: managed → operator QWEN_API_KEY (fail closed with
...
export async function handleQwen(
  request: Request,
  env: Env,
  ...
  // Step 6 — entitlement gate (fail-closed). Managed → operator QWEN_API_KEY.
  const accountMode = (record.account_mode ?? "managed") as "managed" | "byok";
  const keyResolution = resolveUpstreamKey(accountMode, request.headers, env.QWEN_API_KEY ?? null);
  if (!keyResolution.ok) {
  ...
  const upstreamUrl = env.QWEN_BASE_URL ?? "https://dashscope-intl.aliyuncs.com/compatible-mode";
